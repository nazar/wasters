...
module MiniMagick
  ...
  class Image
    ...
    def crop(x,y,w,h,outfile)
      dims = "#{w}x#{h}+#{x}+#{y}"
      run_command('convert', @path, '-crop', dims, '+repage', outfile)
    end

    def crop_self(x,y,w,h)
      crop(x,y,w,h,@path)
    end
    ...
    def run_command(command, *args)
      args.collect! do |arg|
        arg = arg.to_s
        arg = %|"#{arg}"| unless (arg =~ /^-|^\+|^caption|format/) # values quoted because they can contain characters like '>', but don't quote switches
        arg
      end
