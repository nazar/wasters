(function($) {
  ...
  $.fn.extend({
    idFromLink: function() {
      if (this.attr('nodeName') == 'FORM') {
        link_attr = 'action';
      } else { link_attr = 'href'; }
      if (this.attr(link_attr)) {
        return this.attr(link_attr).match(/(\d+)$/g);
      } else {return ''}
    }
  });
  ...
  //          $.fbDebug(function(){console.debug('script: %o', script)});
        ...
        } else { //hmm... no script tags... try an evaling the body
          request._evalScript(data);
        }
      }
      return false;
    },
  ...
  Remote.LinkLive = $.klass(Remote.Link, {
    initialize: function($super, options) {
      options = $.extend({evalScripts: true, data: 'ptype=jq&'+options.data}, options || {});
      $super(options);
    }
  });
